 ...
 * 
 * Features:
 * - Dungeon-generatie gebaseerd op een graaf (graph) met behulp van adjacentielijsten
 * - Kamers bevatten monsters, items, schatten of zijn leeg
 * - De speler navigeert tussen kamers en vecht tegen monsters met bitwise-gevechtsmechanismen
 * - Items hebben effecten zoals levenspunten herstellen of schade verhogen
 * - Spel opslaan en laden naar/van een bestand
 * - Gebruik van functie-pointers voor het verwerken van kamertoegang
 * - Dynamische geheugenallocatie en correcte opruiming van geheugen
 * - Command-line args: <num_rooms> for new game or <filename> to load
 * 
 * compile gcc dungeon_crawler.c -o dungeon_crawler.exe
 */
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf("Gebruik: %s <aantal_kamers> OF %s <savefile>\n", argv[0], argv[0]);
        return 1;
    }
    ...
    if (*endptr == '\0' && num > 1) {
        ...
        if (!rooms) {
            fprintf(stderr, "Fout bij laden van '%s'\n", argv[1]);
            return 1;
        }
    }
    ...
    while (1) {
        ...
        printf("Beschikbare deuren:");
        for (RoomListNode* it = cur->neighbors; it; it = it->next)
            printf(" %d", it->room->id);
        printf("\nKies een deur (-1 om op te slaan en te stoppen): ");
        ...
        if (choice == -1) {
            ...
            printf("Spel opgeslagen. Tot de volgende keer!\n");
            break;
        }
